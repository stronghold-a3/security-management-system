...
import { supabase } from '@/lib/supabase'; // <--- IMPORTS YOUR NEW CREDENTIALS
...
const OpsDashboard: React.FC<{ onBroadcast: () => void; sites?: string[]; canBroadcast?: boolean }> = ({ onBroadcast, sites, canBroadcast = true }) => {
  ...
  useEffect(() => {
    ...
    // Realtime subscription: Instantly updates the dashboard when a new lead comes in
    const channel = supabase
      .channel('leads-db-changes')
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'security_assessment_leads' }, (payload) => {
